...
const int N = 9973, M = 1229, L = 1001;
...
int main(){
    ...
    while (~scanf("%d%d%d", &t, &n, &MOD)){
        if (t == 1) OT(pow(n, n - 2));
        else if (t == 2) OT(pow(n, n - 1));
        else {
            ...
            FOR(i, 1, n){
                REP_1(j, i){
                    S[i][j] = sum(S[i-j][j], A[i+1-j]);
                    INC(A[i+1], pdt(pdt(j, A[j]), S[i][j]));
                }
                DIV(A[i+1], i);
            }
            ...
            if (t == 3) OT(A[n]);
                ...
                REP_1(i, n >> 1)
                    DEC(z, pdt(A[i], A[n-i]));
                if (!(n&1)) INC(z, ((A[n/2]+1) * A[n/2] / 2) % MOD);
                OT(z);
            }
        }
    }
